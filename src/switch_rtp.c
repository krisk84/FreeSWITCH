														  ...
														  uint32_t index, switch_rtp_crypto_key_type_t type, unsigned char *key, switch_size_t keylen)
{
	...
	case AEAD_AES_GCM_256_8:
		crypto_policy_set_aes_gcm_256_8_auth(&policy->rtp);
		crypto_policy_set_aes_gcm_256_8_auth(&policy->rtcp);

		if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND) {
			switch_channel_set_variable(channel, "rtp_has_crypto", "AEAD_AES_GCM_256_8");
		}
		break;

	case AEAD_AES_GCM_128_8:
		crypto_policy_set_aes_gcm_128_8_auth(&policy->rtp);
		crypto_policy_set_aes_gcm_128_8_auth(&policy->rtcp);

		if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND) {
			switch_channel_set_variable(channel, "rtp_has_crypto", "AEAD_AES_GCM_128_8");
		}
		break;
